function [predError, predCov] = predictError(lambda, possNewError, noiseCov, n)
    ...
    %   error (pre-update).
    predError = (lambda*possNewError(:,2*n+1)+ 0.5*sum(possNewError(...
		:,1:2*n),2))/(n+lambda);
    ...
    predCov = (lambda*(possNewError(:,2*n+1)-predError)*(transpose(...
		possNewError(:,2*n+1)-predError)) + 0.5*P_in*transpose(P_in))/...
		(n+lambda) + noiseCov;
end
